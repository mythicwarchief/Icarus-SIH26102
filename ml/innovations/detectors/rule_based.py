...
RULES = {
    ...
    "R11": {
        "name": "Cost Outside Expected Range",
        "severity": "medium",
        "category": "financial",
        "description": "Sanction amount falls outside the expected cost range for similar projects",
    },
}
...
def detect(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # R11: Cost Outside Expected Range
    cost_in_range = df.get("cost_in_expected_range", pd.Series(True, index=df.index))
    cost_deviation = df.get("cost_deviation_pct", pd.Series(0.0, index=df.index))
    _check_rule(
        df, all_triggered, all_explanations, all_severities, all_categories,
        rule_id="R11",
        mask=(
            (~cost_in_range.astype(bool))
            & (cost_deviation > 20)  # Only flag if >20% outside range
        ),
        explanation_fn=lambda row: (
            f"This project's sanction amount of Rs.{row.get('sanction_amount', 0):,.0f} "
            f"is {row.get('cost_deviation_pct', 0):.1f}% outside the expected cost range "
            f"of Rs.{row.get('expected_cost_low', 0):,.0f} - Rs.{row.get('expected_cost_high', 0):,.0f} "
            f"for similar projects ({row.get('comparison_group', 'N/A')}, "
            f"based on {row.get('comparison_group_size', 0):.0f} historical projects)."
        ),
    )
