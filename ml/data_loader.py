...
def _drop_footer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = df.iloc[:, 0].astype(str).str.strip().isin(
        config.GRAND_TOTAL_MARKERS
    )
...
def load_sanctioned() -> pd.DataFrame:
    ...
    # Parse dates
    df["recommended_date"] = _parse_date_flexible(
        df["recommended_date"]
    )

    df["sanction_date"] = _parse_date_flexible(
        df["sanction_date"]
    )
...
def load_completed() -> pd.DataFrame:
    ...
    df["completion_date"] = _parse_date_flexible(
        df["completion_date"]
    )
...
def load_expenditure() -> pd.DataFrame:
    ...
    # Parse expenditure date
    df["expenditure_date"] = _parse_date_flexible(
        df["expenditure_date"]
    )
...
def _aggregate_expenditure(
    ...
    agg = exp.groupby(
        ...
        exp_first_payment_date=(
            "expenditure_date",
            "min"
        ),

        exp_last_payment_date=(
            "expenditure_date",
            "max"
        ),
    ...
    vendor_shares = vendor_shares.merge(
        vendor_totals,
        on="work_id"
    )

    vendor_shares["share"] = (
        vendor_shares["amount"]
        / vendor_shares["vendor_total"]
    )

    vendor_shares["share_sq"] = (
        vendor_shares["share"] ** 2
    )
...
def build_master_dataset() -> pd.DataFrame:
    ...
    print(
        "[DataLoader] Loading sanctioned works..."
    )

    sanctioned = load_sanctioned()

    print(
        f"  -> {len(sanctioned)} rows, "
        f"{sanctioned['work_id'].notna().sum()} "
        f"with work_id"
    )

    print(
        "[DataLoader] Loading completed works..."
    )

    completed = load_completed()

    print(
        f"  -> {len(completed)} rows, "
        f"{completed['work_id'].notna().sum()} "
        f"with work_id"
    )

    print(
        "[DataLoader] Loading features..."
    )
    ...
    print(
        "[DataLoader] Loading expenditure..."
    )

    expenditure = load_expenditure()

    print(
        f"  -> {len(expenditure)} rows"
    )

    print(
        "[DataLoader] Aggregating expenditure by work_id..."
    )

    exp_agg = _aggregate_expenditure(
        expenditure
    )

    print(
        f"  -> {len(exp_agg)} unique works "
        f"with expenditure"
    )
    ...
    print(
        f"[DataLoader] Base master: "
        f"{len(master)} sanctioned works with work_id"
    )

    # Deduplicate completed works
    completed_dedup = completed[
    ...
    # Create a set of completed work_ids
    completed_ids = set(
        completed_dedup[
            "work_id"
        ].unique()
    ...
    def _assign_status(row):
        ...
        if wid in completed_ids:
            ...
            # Check if there's any expenditure
            if (
                pd.notna(
                    row.get(
                        "exp_total_amount"
            ...
            # Default:
            # If expenditure exists -> Ongoing
            # Otherwise -> To Be Implemented
    ...
    print(
        f"[DataLoader] Master dataset built: "
        f"{len(master)} works"
    )

    print(
        "  Status distribution:\n"
        f"{master['status_category'].value_counts().to_string()}"
    )
